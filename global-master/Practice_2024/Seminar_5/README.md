...
* Суть оконных функций заключается в разделении запроса на партиции по заданным условиям. При этом каждая часть обрабатывается отдельно от других. Функция выдаёт отдельный результат для каждой строки, сохраняя его. Этим оконные функции отличаются от GROUP BY, где строки группируются.

![](img/Screenshot 2024-12-29 at 13.24.56.png)
...
### 2. Необязательная часть для тех, кто знаком с CTE.

1. Определите среднее и среднеквадратичное отклонение цен для каждого продукта в окне из текущего и двух предыдущих продуктов, чтобы провести анализ отклонений.

2. Подсчитайте количество уникальных цен продуктов в пределах окна из последних 5 цен в группе.
